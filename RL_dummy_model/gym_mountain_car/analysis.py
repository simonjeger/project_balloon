...
def plot_path():
    ...
    list_rew_epi = [[] for i in range(n+1)]
    for i in range(len(epi)):
        ...
        list_rew_epi[epi[i]].append(rew_epi[i])

    lim_rew = [min(rew_epi), max(rew_epi)]
    ...
    for n in range(len(list_pos_x)):
        ...
        # plot reward
        axs[1].plot(np.arange(0,len(list_rew_epi[n]),1), list_rew_epi[n])
        axs[1].set_ylim(lim_rew[0]*1.1,lim_rew[1]*1.1)
