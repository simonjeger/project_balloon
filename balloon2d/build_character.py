...
class character():
    ...
    def set_state(self):
        ...
        if yaml_p['type'] == 'regular':
            boundaries = np.concatenate(([self.position[1]],self.compress_terrain()/[self.size_x, self.size_z], [self.position[0] - np.floor(self.position[0])]))
            ...
            rel_pos = self.height_above_ground()/(self.ceiling-(self.position[1]-self.height_above_ground()))
            total_z = (self.ceiling-(self.position[1]-self.height_above_ground()))/self.size_z
            boundaries = np.array([rel_pos, total_z, self.position[0] - np.floor(self.position[0])])
