...
class Agent:
    ...
    def set_reachable_target(self):
        ...
        while True:
            ...
            dist_to_start = np.sqrt(((self.env.character.position[0] - self.env.character.start[0])*self.render_ratio/yaml_p['radius_xy'])**2 + ((self.env.character.position[1] - self.env.character.start[1])*self.render_ratio/yaml_p['radius_xy'])**2 + ((self.env.character.position[2] - self.env.character.start[2])/yaml_p['radius_xy'])**2)
