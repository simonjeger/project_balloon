...
class balloon2d(Env):
    ...
    def reset(self):
        ...
        self.character = character(self.size_x, self.size_y, self.size_z, start, target, self.T, self.world, self.world_compressed)
    ...
    def set_target(self, start):
        ...
        dist = 0
        dist_max = 0
        for c in [[0,0,0], [0,0,self.size_z], [0,self.size_y,0], [0,self.size_y,self.size_z], [self.size_x,0,0], [self.size_x,0,self.size_z], [self.size_x,self.size_y,0], [self.size_x,self.size_y,self.size_z]]:
            dist = np.sqrt((start[0] - c[0])**2 + (start[1] - c[1])**2 + (start[2] - c[2])**2)
            if dist > dist_max:
                dist_max = dist

        # correct target if needed
        dist_now = np.sqrt((start[0] - target[0])**2 + (start[1] - target[1])**2 + (start[2] - target[2])**2)
        if dist_now > dist_max*curr:
            dir_norm = [target[0] - start[0], target[1] - start[1],  target[2] - start[2]] / dist_now
