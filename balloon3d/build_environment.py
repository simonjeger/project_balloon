...
class balloon2d(Env):
    def __init__(self, epi_n, step_n, train_or_test, writer=None, radius_z=yaml_p['radius_xy']):
        ...
        if yaml_p['physics']:
            regular_state_space_low = np.array([-self.size_x,-self.size_y,-self.size_z,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf,-self.size_x,-self.size_y,-self.size_z]) #residual to target, velocity, measurement, distance to border
            regular_state_space_high = np.array([self.size_x,self.size_y,self.size_z,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,self.size_x,self.size_y,self.size_z])
        else:
            regular_state_space_low = np.array([-self.size_x,-self.size_y,-self.size_z,-np.inf,-np.inf,-np.inf,-self.size_x,-self.size_y,-self.size_z]) #residual to target, measurement, distance to border
